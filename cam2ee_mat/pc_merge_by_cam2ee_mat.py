...
def euler_to_rotation_matrix(rx, ry, rz, isDegree):
    # Create a rotation object using the Euler angles in ZYX order
    rotation = R.from_euler('xyz', [rx, ry, rz], degrees=isDegree)
...
# def merge_pc(c2f_mat, pc_A, A_mat, pc_B, B_mat, ext_mat):
def merge_pc(c2f_mat, pc_A, A_mat, pc_B, B_mat):
    # cA_T_cB = ext_mat @ np.linalg.inv(c2f_mat) @ np.linalg.inv(A_mat) @ B_mat @ c2f_mat @ np.linalg.inv(ext_mat)
    cA_T_cB = np.linalg.inv(c2f_mat) @ np.linalg.inv(A_mat) @ B_mat @ c2f_mat
...
if __name__ == "__main__":
    ...
    # ext_mat       = config['ext_mat']

    flange_poses  = config['flange_pose_euler']
    flange_mats = []
    # pre_flange_mat = np.array([])
    for vec in flange_poses:
        flange_rot_mat = euler_to_rotation_matrix(vec[3], vec[4], vec[5], True)
        # print(f"Rot Matrix:\n{flange_rot_mat}")
    ...
    for pc_file_path, flange_mat in zip(pc_file_paths, flange_mats):
        ...
        # pc_merged = merge_pc(cam2flange_mat, pc_merged, mat_ref, pc_to_be_transformed, np.array(flange_mat), ext_mat)
